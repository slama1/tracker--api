...
class EngineReportingAgent {
    ...
    startSchemaReporting({ executableSchemaId, executableSchema, }) {
        ...
        if (this.options.experimental_schemaReportingInitialDelayMaxMs !== undefined) {
            this.logger.info(`Schema reporting max initial delay override: ${this.options.experimental_schemaReportingInitialDelayMaxMs} ms`);
        }
        ...
        const delay = Math.floor(Math.random() *
            (this.options.experimental_schemaReportingInitialDelayMaxMs || 10000));
        const schemaReporter = new schemaReporter_1.SchemaReporter(serverInfo, executableSchema, this.apiKey, this.options.schemaReportingUrl);
    ...
    getTraceSignature({ queryHash, operationName, document, source, logger, }) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            this.signatureCache.set(cacheKey, generatedSignature)
                .catch(err => {
                logger.warn('Could not store signature cache. ' +
                    (err && err.message) || err);
            });
